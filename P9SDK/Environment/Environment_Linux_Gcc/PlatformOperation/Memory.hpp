...
_INLINE_ void* memory_alloc(u64 sz)
{
    void* p = reinterpret_cast<void*>(LinuxGccNs::malloc(sz));
    assert(p);
    return p;
}
...
{
    assert(addr);
    LinuxGccNs::free(addr);
}

_INLINE_ void memory_set(void* addr, u8 val, u64 sz)
{
    LinuxGccNs::memset(addr, val, sz);
}

_INLINE_ void memory_copy(const void* src, void* dst, u64 sz)
{
    LinuxGccNs::memcpy(dst, src, sz);
}
