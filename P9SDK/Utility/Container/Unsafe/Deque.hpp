...
public:
    ...
    void push_back(const Ty& e) noexcept
    {
        if (last->full())
        {
            ...
            new (new_blk) Blck_Ty(_clamp(sz, deque_block_min_size, deque_block_max_size), last, nullptr, -1, 0);
            last->mynext() = new_blk;
            last = new_blk;
        }
        last->push_back(e);
    ...
    void push_front(const Ty& e) noexcept
    {
        ...
        {
            Blck_Ty* new_blk = (Blck_Ty*)memory_alloc(sizeof(Blck_Ty));
            s64 new_cap = _clamp(sz, deque_block_min_size, deque_block_max_size);
