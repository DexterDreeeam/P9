...
protected:
    hashmap() noexcept :
        ...
        sz(0),
        slots(memory::alloc_zero<BaseNode_Ty*>(sizeof(void*) * maplen))
    {
        assert(_self_exam());
    }
    ...
    hashmap(s64 expected_maplen) noexcept :
        allocator(),
        maplen(math::max(expected_maplen, hashmap_least_maplen)),
        sz(0),
        slots(memory::alloc_zero<BaseNode_Ty*>(sizeof(void*) * maplen))
    {
        assert(_self_exam());
        ...
        slots(memory::alloc_zero<BaseNode_Ty*>(sizeof(void*) * maplen))
    {
        ...
        memory::set(rhs.slots, 0, sizeof(void*) * maplen);
        rhs.sz = 0;
        assert(_self_exam());
    }
    ...
    bool _reorg(s64 new_maplen) noexcept
    {
        ...
        BaseNode_Ty** new_slots = memory::alloc_zero<BaseNode_Ty*>(sizeof(void*) * new_maplen);
