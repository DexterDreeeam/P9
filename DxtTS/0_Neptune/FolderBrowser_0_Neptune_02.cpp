...
string search_string = ""; //# vacancy is for disable
...
void func_search_string(string &file_path, string &search);
...
void iterate_file(string file_path)
{
    ...
    {
        func_search_string(file_path, search_string);
    }
}
...
void func_search_string(string &file_path, string &search)
{
    ...
    int lenOfSearch = search.size();
    vector<pair<int, string>> vp; //# line, content

    while (!feof(f))
    {
        fgets(char_buf, sizeof(char_buf) - 1, f);
        string s(char_buf);

        for (int i = 0; i < s.size(); ++i)
        {
            string part = s.substr(i, lenOfSearch);
            if (part.size() < lenOfSearch)
            {
                break;
            }
            if (part == search)
            {
                ...
                vp.push_back(make_pair(line, s)); //# found
                break;
            }
        }
...
int main(int argc, const char *args[])
{
    ...
    //# input whether search specific string
    input.clear();
    cout << "Do you want \"Search String\" operation ?" << " enter Y/N :";
    getline(cin, input);
    if (input == "Y" || input == "y")
    {
        cout << "\"Search String\" ENABLED." << endl;

        cout << "What string you want to search : ";
        getline(cin, search_string);
    }
    else
    {
        cout << "\"Search String\" DISABLED." << endl;
